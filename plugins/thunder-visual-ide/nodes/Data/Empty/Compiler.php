...
final class EmptyCheckCompiler implements NodeCompilerInterface
{
    public function compile(array $node, CompileContext $context): array
    {
        $nodeId = (string) ($node['id'] ?? '');
        $hasConnectedValue = $context->incomingEdges($nodeId, 'value') !== [];

        if ($hasConnectedValue) {
            $value = $context->inputExpression($nodeId, 'value', 'null');
            $expression = 'empty(' . $value . ')';
        } else {
            $target = $this->target($node, $context, 'Empty Check');
            $expression = $target !== null ? 'empty(' . $target . ')' : 'true';
        }
