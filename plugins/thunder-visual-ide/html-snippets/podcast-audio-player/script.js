...
root.querySelector('[data-skip-back]')?.addEventListener('click', () => {
    if (media) {
        media.currentTime = Math.max(0, media.currentTime - 15);
    }
});

const followButton = root.querySelector('[data-follow-podcast]');
...
followButton?.addEventListener('click', () => {
    ...
    followButton.textContent = isFollowing ? 'Follow show' : 'Following';
});
